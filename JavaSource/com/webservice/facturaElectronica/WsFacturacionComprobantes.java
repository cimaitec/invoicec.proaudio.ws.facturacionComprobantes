...
{	
												  ...
												  @WebParam(name="TipoDocumento") String p_tipoDocumento)
	{
		...
		try
		{
			...
			{
				String ls_estado, ls_descripcion, ls_autorizacion, ls_fechaAutorizacion;
				int index=0;
				...
				ls_respuesta = ls_respuesta.substring(index+1, ls_respuesta.length());
				index = ls_respuesta.indexOf('|');
				ls_autorizacion = ls_respuesta.substring(0, index);
				
				ls_respuesta = ls_respuesta.substring(index+1, ls_respuesta.length());
				index = ls_respuesta.indexOf('|');
				ls_fechaAutorizacion = ls_respuesta.substring(0, index);
				//
				ls_respuesta = ls_respuesta.substring(index+1, ls_respuesta.length());
				
				rp.setEstadoDocumento(ls_estado);
				rp.setDescripcion(ls_descripcion);
				rp.setAutorizacion(ls_autorizacion);
				rp.setFechaAutorizacion(ls_fechaAutorizacion);
